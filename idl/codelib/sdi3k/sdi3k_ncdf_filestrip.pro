...
pro sdi3k_ncdf_filestrip, data_dir, temp_dir, archive_dir, stripped_dir, filter=filter
	...
	top = 'C:\cal\StripTest\'
	data_dir = top + 'StripData\'
	temp_dir = top + 'StripTemp\'
	archive_dir = top + 'StripArchive\'
	stripped_dir = top + 'StripStrip\'

	if not(keyword_set(filter)) then filter = ['*.pf', '*.nc', '*.sky', '*.las']

	flis = file_search(data_dir + filter)
	for j=0,n_elements(flis)-1 do begin
	    ...
	    file_move, strip_name, stripped_dir
